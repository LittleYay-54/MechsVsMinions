...
def tuple_to_vector(input_tuple: tuple) -> Vector:
    ...
    :return: 2x1 vector
    """
    return np.reshape(input_tuple, (2,))
...
def rotate(input_vector: Vector, angle: int) -> Vector:
    """
    Performs a linear transformation that rotates a 2x1 vector counterclockwise about the origin. Input only right angles.
    :param input_vector: 2x1 vector
...
def oob_check(board: Board, location: Vector) -> bool:
    ...
    """
    if location[0] < 0 or location[0] > board.board_array.shape[0] - 1:
        return False
    elif location[1] < 0 or location[1] > board.board_array.shape[1] - 1:
        return False
    else:
        return True
